...
class AgencyController extends Controller
{
    ...
    public function store(StoreAgencyRequest $request): AgencyResource|JsonResponse
    {
        /** @var User $user */
        $user = $request->user();
        $name = trim($request->string('name')->toString());
        $normalizedName = Str::of($name)->lower()->squish()->replaceMatches('/[^a-z0-9]+/', '-')->trim('-')->toString();
        ...
        if (AgencyCompany::query()->where('normalized_name', $normalizedName)->exists()) {
            return ApiError::conflict('An agency/company with this name already exists.');
        }
